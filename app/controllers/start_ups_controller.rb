class StartUpsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @start_up.save
        ...
        format.json { render action: 'show', status: :created, location: @start_up }
      else
  ...
  def destroy
    @start_up.destroy
    respond_to do |format|
      format.html { redirect_to start_ups_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def set_start_up
      @start_up = StartUp.friendly.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def start_up_params
      params.require(:start_up).permit(:name, :description)
    end
end
